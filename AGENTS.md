...
- `bin/omarchy-arcade-scan` reads RetroArch playlists and streams tagged NDJSON lines to stdout — an internal first-seen document, then header, one `{"t":"game"}` line per game, and a trailer; playlist metadata is the library source. It forks jq once per playlist, streaming NUL-delimited fields, and its helpers return results through variables (`CORE_PATH`, `EXT`, `BOXART`, `STATE_INFO`) instead of stdout. Do not reintroduce per-ROM process spawns or `$( )` captures of these helpers: they cost ~7ms each and turned a 400-game scan from 150ms into seconds. Invocations serialize on an exclusive state-dir flock, so panel and overlay can never enumerate concurrently.
